...
stop_existing_instances() {
  ...
  if [[ -f "$PID_FILE" ]]; then
    ...
    if [[ -n "$pidfile_pid" ]]; then
      stop_pid "$pidfile_pid" "pidfile rcod" || return 1
    fi
    rm -f "$PID_FILE"
  fi
  ...
  while IFS= read -r pid; do
    ...
    found_running=true
    stop_pid "$pid" "existing rcod for $CONFIG_PATH" || return 1
  done < <(find_existing_pids)

  if [[ "$found_running" == true ]]; then
    echo "previous rcod instances stopped"
  fi
}
...
env \
  ...
  "$GO_BIN" build -o "$BIN_PATH" ./cmd/rcod
...
if kill -0 "$STARTED_PID" 2>/dev/null; then
  echo "rcod started (pid $STARTED_PID)"
  echo "log: $LOG_FILE"
  exit 0
fi

echo "rcod failed to stay up; see $LOG_FILE" >&2
exit 1
